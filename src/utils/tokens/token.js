...
// Average length of output SVG
// ${generateCirlceBgSVG(year)} = 2000
// ${generateDateStringSVG(year, month, day)} = 1000
// ${generateDaySVG(day)}  =  2500
// ${generateBarcodeSVG(year, month, day)} = 4000
// ${generateTokenIdSVG(tokenId)}  =  500
// ${generateYearCircleSVG(year)}  =  7500
// ${generateMonthSVG(month, false, 180, 0.25, '30s', 0.5)}  =  1000
...
function generateCirlceBgSVG(year, center_x = 175, center_y = 133, from = 95, distance = 10, num = 12) {
	...
	for (let i = 0; i < num; i++) {
		...
		result += `<circle r="${from + i * distance}" style="fill: none; stroke: url(#color-0); stroke-width: ${hot ? 0.6 : 0.3}; opacity: ${
			hot ? 0.5 : 0.2
		};"/>\n`;
